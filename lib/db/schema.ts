...
export const usersTable = pgTable('users_table', {
  ...
  profileJson: jsonb('profile_json').$type<{ name?: string; phone?: string; deliveryPreference?: string }>().default({}),
...
export const userAddressesTable = pgTable('user_addresses_table', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  cityOrDistrict: varchar('city_or_district', { length: 120 }).notNull(),
  state: varchar('state', { length: 120 }).notNull(),
  pinCode: varchar('pin_code', { length: 20 }).notNull(),
  deliveryPreference: varchar('delivery_preference', { length: 120 }).default('Standard dispatch'),
  isDefault: boolean('is_default').notNull().default(false),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({
  userAddressIdx: index('user_addresses_user_id_idx').on(table.userId),
}));

export const paymentGatewayConfigTable = pgTable('payment_gateway_config_table', {
  id: uuid('id').primaryKey().defaultRandom(),
  gatewayName: varchar('gateway_name', { length: 40 }).notNull().unique(),
  enabled: boolean('enabled').notNull().default(false),
  configJson: jsonb('config_json').$type<Record<string, string | boolean | number>>().default({}),
