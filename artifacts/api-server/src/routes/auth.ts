...
authRouter.post('/verify-otp', (req, res) => {
  ...
  const token = `vmp_${crypto.randomUUID()}`;
  res.cookie('vmp_b2b_session', token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 1000 * 60 * 60 * 24 * 30 });
  return res.json({
    ok: true,
    session: {
