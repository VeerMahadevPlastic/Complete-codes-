...
async function dispatchAdministrativeOrder(payload: EnterpriseOrderPayload) {
  const administrativeWebhookUrl = process.env.VMP_ADMIN_COMMUNICATION_WEBHOOK_URL;
  const dispatchBody = {
    channel: "system-whatsapp-admin-network",
    payload,
    trace: "VMP Core: Workspace matched with premium live production specifications.",
  };

  if (!administrativeWebhookUrl) {
  ...
  await fetch(administrativeWebhookUrl, {
    method: "POST",
...
router.post("/enterprise-order", async (req, res, next) => {
  try {
    ...
    dispatchAdministrativeOrder(payload).catch((error: unknown) => {
      console.error("VMP admin communication dispatch failed", error);
    });
