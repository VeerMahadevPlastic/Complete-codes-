...
export function AuthModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const completeVerification = async () => {
    ...
    const response = await fetch('/api/auth/verify-otp', {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ businessName, emailOrPhone: identity, otp: code }),
    }).catch(() => null);
    const payload = response?.ok ? await response.json() : null;
    setSession(payload?.session ?? { businessName, emailOrPhone: identity, phone: identity, token: `vmp_${crypto.randomUUID()}`, verifiedAt: new Date().toISOString() });
    onClose();
  };
