...
import { siteBranding } from '../../lib/branding';

export function Header() {
  const { products, categories, currency, setCurrency, cart, session, setSession } = useStore();
    ...
    <>
      <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/90 text-white shadow-2xl shadow-slate-950/10 backdrop-blur-xl backdrop-saturate-150">
        <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 md:gap-5">
          <a href="/" className="flex shrink-0 items-center gap-2 text-lg font-black tracking-tight"><img src={siteBranding.logoUrl} alt="" className="h-9 w-9 rounded-xl bg-white p-1" /><span className="hidden sm:inline">{siteBranding.websiteName}</span></a>
          <button className="hidden rounded-2xl px-3 py-2 text-left text-xs leading-tight transition hover:bg-white/10 lg:block"><span className="block text-white/60">Deliver to</span><strong>{session?.defaultAddress?.pinCode ?? 'Set Pincode'}</strong></button>
          <div id="search" className="relative flex-1">
            <div className="flex h-12 overflow-hidden rounded-2xl bg-white text-slate-950 ring-emerald-400/40 transition focus-within:ring-4"><select aria-label="Search category" className="hidden w-36 border-r border-black/10 bg-slate-100 px-3 text-sm font-bold outline-none md:block"><option>All</option>{categories.map((category) => <option key={category}>{category}</option>)}</select><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search products, categories, item codes..." className="min-w-0 flex-1 px-4 outline-none" /></div>
            {query && (
          ...
          <a href="/my-orders.html" className="hidden rounded-2xl px-3 py-2 text-xs font-bold transition hover:bg-white/10 lg:block">Returns<br/><span className="text-sm">& Orders</span></a>
          <a href="/#wishlist" className="hidden rounded-2xl px-3 py-2 text-xs font-bold transition hover:bg-white/10 xl:block">Wishlist</a>
          <button onClick={() => setCartOpen(true)} className="relative h-12 rounded-2xl bg-emerald-500 px-4 font-black text-white transition duration-250 ease-premium hover:scale-[1.03] transform-gpu">Inquiry<span className="ml-2 rounded-full bg-white px-2 py-0.5 text-xs text-emerald-700">{cart.length}</span></button>
          {!session ? (
